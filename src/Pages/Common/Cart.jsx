...
import { removeProduct, resetCart } from '../../Components/Redux/counterSlice'
            ...
            <div className='absolute bg-white z-30 right-0 p-5 shadow-lg shadow-blue-200 max-w-sm'>
                {
                    products.length > 0 ?
                        <>
                            ...
                            <div className='grid gap-5'>
                                {
                                    products?.map(item => <div className='' key={item.id}>
                                        <div className='flex justify-between gap-5 items-center border-b-2 pb-5'>
                                            ...
                                            <AiOutlineDelete onClick={() => dispatch(removeProduct(item.id))} className='text-red-500 text-2xl cursor-pointer min-w-[40px]' />
                                        </div>
                                    </div>)
                                }
                            </div>
                            ...
                            <button onClick={() => dispatch(resetCart())} className='text-red-500 pt-7'>Reset Cart</button>
                        </>
                        :
